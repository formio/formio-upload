...
module.exports = function authenticate(req, res, next) {
  ...
  if (req.query.token) {
    ...
    request.get({
      ...
      const userTeams = userBody.teams;
      const userRoleIds = userBody.roles;
      ...
      if (userBody.project === process.env.PORTAL_BASE_PROJECT_ID) {
        ...
        request.get({
          ...
          const teamAccess = projectBody.access.filter(access => access.type === 'team_access' || access.type === 'team_admin' );
          const teams = teamAccess[0].roles;
          ...
          const intersect = userTeams.filter(value => teams.includes(value));
        ...
        request.get({
          ...
          const writeAccess = formBody.submissionAccess.filter(access => access.type === 'create_own' || access.type === 'create_all');
          const writeAccessRoles = [];

          for (const access of writeAccess) {
          ...
          const intersect = userRoleIds.filter(value => writeAccessRoles.includes(value));
